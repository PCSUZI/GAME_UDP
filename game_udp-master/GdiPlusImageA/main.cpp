                     ...
                     LPSTR lpCmdLine,     int nCmdShow)
{
	...
	HWND hwnd = CreateWindow("HelloWin", "친구랑 하는 게임 진까리!!", 
                              WS_OVERLAPPEDWINDOW,
	...
	pSound[0] = pSoundManger->LoadSound("오프닝.wav", true);
	pSound[1] = pSoundManger->LoadSound("클릭.wav", false);
	pSound[2] = pSoundManger->LoadSound("초성게임.wav", true);
	pSound[3] = pSoundManger->LoadSound("가위바위보.wav", true);
	pSound[4] = pSoundManger->LoadSound("정답.wav", false);
	pSound[5] = pSoundManger->LoadSound("오답.wav", false);
	pSound[6] = pSoundManger->LoadSound("승리.wav", false);
	pSound[7] = pSoundManger->LoadSound("주사위.wav", true);
	pSound[8] = pSoundManger->LoadSound("큰이동.wav", false);
	pSound[9] = pSoundManger->LoadSound("이동.wav", false);
...
BOOL CreateSocket(HWND hWnd)
{
	...
	serveraddr.sin_port = htons(30000);//나는 삼만번
	...
	g_ToClientaddr.sin_port = htons(40000); //상대방은 사만번
